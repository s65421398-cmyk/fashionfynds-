...
export function ShopProvider({ children }: { children: React.ReactNode }) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          trackEcommerce('add_to_cart', {
            items: [{ item_id: product.id, item_name: product.name, price: product.price, quantity: 1, currency: 'INR', item_brand: product.brand, item_category: product.category }],
            value: product.price,
            currency: 'INR',
          });
        } else {
  ...
  const removeFromCart = async (id: string, size: string, color: string) => {
    if (session?.user) {
      const cartData = await fetch("/api/cart").then(r => r.json());
      const itemToRemove = cartData.find((item: any) =>
        String(item.product.id) === id && item.selectedSize === size && item.selectedColor === color
      );
      if (itemToRemove) {
        try {
          const res = await fetch(`/api/cart/${itemToRemove.id}`, { method: "DELETE" });
          if (res.ok) {
            await loadFromDatabase();
  ...
  const updateCartQuantity = async (id: string, size: string, color: string, quantity: number) => {
    if (quantity <= 0) {
    ...
    if (session?.user) {
      ...
      if (itemToUpdate) {
        try {
          ...
          if (res.ok) await loadFromDatabase();
        } catch {
    ...
    if (session?.user) {
      try {
        ...
        if (res.ok) {
          ...
          toast.error(data.code === "DUPLICATE_ITEM" ? "Already in wishlist" : "Failed to add to wishlist");
        }
      } catch {
  ...
  const removeFromWishlist = async (id: string) => {
    if (session?.user) {
      const wishlistData = await fetch("/api/wishlist").then(r => r.json());
      const itemToRemove = wishlistData.find((item: any) => String(item.product.id) === id);
      if (itemToRemove) {
        try {
          const res = await fetch(`/api/wishlist/${itemToRemove.id}`, { method: "DELETE" });
          if (res.ok) {
            await loadFromDatabase();
